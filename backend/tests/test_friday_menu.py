...
# Ensure backend is on sys.path so we can import the local scraper.
BACKEND_ROOT = Path(__file__).resolve().parents[1]
if str(BACKEND_ROOT) not in sys.path:
    sys.path.insert(0, str(BACKEND_ROOT))

school_lunch_checker = importlib.import_module("school_lunch_checker")


@pytest.fixture
def menu_module():
    """Provide the local LunchMenuChecker implementation."""
    return school_lunch_checker
